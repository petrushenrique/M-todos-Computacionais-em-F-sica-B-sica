...
A Fig. \ref{fig:anatomy} mostra as diversas partes de uma figura.
...
\begin{enumerate}
    ...
    \item {\bf Eixos}: Um Eixo é um objeto anexado a uma Figura que contém uma região para plotagem de dados e geralmente inclui dois objetos (ou três no caso de 3D) que fornecem marcas e rótulos de marca para fornecer escalas para os dados nos Eixos. Cada Eixo também tem um título (definido por meio de set\textunderscore title()), um rótulo x (definido por meio de set\textunderscore xlabel()) e um rótulo y definido por set\textunderscore ylabel()).

A classe Axes e suas funções de membro são o principal ponto de entrada para trabalhar com a interface OOP e têm a maioria dos métodos de plotagem definidos nelas (por exemplo, ax.plot(), mostrado acima, usa o método plot)
\end{enumerate}
